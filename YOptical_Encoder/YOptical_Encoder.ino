...
void loop() {
  if (oldEncPos != encoderPos) { // if encoder position incremented reset it (idk if this is necessary
...
// at the event the master arduino requests information, parse the encoder position value into a two byte array and send it on the i2c bus
void requestEvent() { 
  byte byteArray[2];
  byteArray[0] = (encoderPos >> 8) & 0xFF;
  byteArray[1] = encoderPos & 0xFF;
